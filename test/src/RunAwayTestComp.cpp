...
{
  RunAwayTestInit(manager);
  RunAwayInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("RunAwayTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("RunAwayTest0");
  if (comp == nullptr)
  {
  ...
  RunAwayTest* testcomp = dynamic_cast<RunAwayTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
